...
def verificar_faixa_ip(primeiro_octeto, segundo_octeto, terceiro_octeto, quarto_octeto):
  ...
  # FRIST OCTET IS THE ONLY ONE THAT CAN'T BE 0
...
def mascara_de_rede(rede_user):
  ...
  for contar_bits in range(rede_user):
    if(contar_bits < limite_bits):
      ...
      # EACH NUMBER BETWEEN 8 AND 16 IS 1 BIT
      contador_bits_segundo_octeto += 1
    elif(contar_bits < limite_bits * 3):
      # EACH NUMBER BETWEEN 16 AND 24 IS 1 BIT
      contador_bits_terceiro_octeto += 1
    else:
      # EACH NUMBER BETWEEN 24 AND 32 IS 1 BIT
      contador_bits_quarto_octeto += 1

  # THE FIRST NUMBER TO SUM THE TOTAL VALUE OF BITS WILL ALWAYS BE 128,
  # AND AS IT ADVANCES FROM ONE BIT TO ANOTHER, IT DIVIDES THE PREVIOUS
  # NUMBER BY 2, UNTIL IT REACHES THE POINT THERE ARE NO MORE BITS TO
  # ADVANCE, OR THE SUM RESULT IS 255
...
def rede_broadcast_ip_valido(primeiro_octeto, segundo_octeto, terceiro_octeto, quarto_octeto, rede_user):
  ...
  if(rede_user < maximo_bits_total//2):
    # HOW MANY BITS ARE ON ON THE 2ND OCTET (CIDR BETWEEN 8 AND 16)
    ...
    # SINCE THE CIDR IS BETWEEN 8 AND 16, THE FIRST IP'S OCTET WILL ALWAYS REPEAT
    ...
    # HOW MANY BITS ARE ON ON THE 3RD OCTET (CIDR BETWEEN 16 AND 24)
    ...
    # SINCE THE CIDR IS BETWEEN 16 AND 24, THE FIRST AND SECOND IP'S OCTETS WILL ALWAYS REPEAT
    ...
    # HOW MANY BITS ARE ON ON THE 4TH OCTET (CIDR BETWEEN 24 AND 32)
    ...
    # SINCE THE CIDR IS BETWEEN 16 AND 24, THE FIRST, SECOND AND THIRD IP'S OCTETS WILL ALWAYS REPEAT
...
def definir_sufixo(number):
  lista_sufixos = []
  sufixo = None

  for posicao in number:
    if(posicao == 1):
      sufixo = 'st'
    elif(posicao == 2):
      sufixo = 'nd'
...
if __name__ == '__main__':
  ...
  while(True):
    ...
    while(True):
      print(f'To exit the program, type "{sair}" and then "{continuar}".' )
      print(f'To do another calculation, just press "{continuar}".')
